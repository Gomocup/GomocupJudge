...
def run_self_play(selected_exe, protocol, rule, board_size, timeout_turn, timeout_match, temp_folder):
    folder_1 = os.path.join(temp_folder, "f1").replace("\\", "/")
    folder_2 = os.path.join(temp_folder, "f2").replace("\\", "/")
    os.makedirs(folder_1, exist_ok=True)
    os.makedirs(folder_2, exist_ok=True)
    
    base_opening = str_to_pos("b2e2e3")
    opening = [(board_size - 1 - x, board_size - 1 - y) for x, y in base_opening]
    cmd = selected_exe.replace("\\", "/")
    working_dir = os.path.dirname(selected_exe).replace("\\", "/")
...
def run_rule_check(selected_exe, protocol, rule, board_size, timeout_turn, timeout_match, temp_folder, sequence_str, rule_type):
    ...
    cmd = selected_exe.replace("\\", "/")
    working_dir = os.path.dirname(selected_exe).replace("\\", "/")
    folder = os.path.join(temp_folder, "check_persistent").replace("\\", "/")
...
def main():
    ...
    parser.add_argument("zip_paths", nargs="+", help="Path to the engine .zip file(s) to test")
    args = parser.parse_args()

    zip_paths = [os.path.abspath(p) for p in args.zip_paths]
    
    valid_paths = []
    for p in zip_paths:
        if not os.path.isfile(p):
            print(f"Warning: ZIP file not found at {p}, skipping.")
        else:
            valid_paths.append(p)
            
    if not valid_paths:
        print("Error: No valid ZIP files to test.")
        sys.exit(1)

    print("==================================================")
    print("Gomocup Engine ZIP Rule Tester")
    print("==================================================")
    print(f"Testing {len(valid_paths)} archives.")
    ...
    for zip_path in valid_paths:
        ...
        print(f"\n==================================================")
        print(f"Testing Archive: {zip_name}")
        print(f"==================================================")
        ...
        try:
            ...
            print(f"\nAn error occurred during testing of {zip_name}:")
            traceback.print_exc()
            for fmt in FORMATS:
                if fmt["name"] not in results:
                    results[fmt["name"]] = (False, f"Testing aborted due to exception: {str(e)}")
        finally:
    ...
    print("\n==========================================================================================")
    print("Final Test Results Report Summary")
    print("==========================================================================================")
    header = f"{'Engine Archive':<22} | {'Freestyle':<10} | {'Fastgame':<10} | {'Freestyle15':<11} | {'Standard':<10} | {'Renju':<10} | {'Caro':<10}"
    print(header)
    print("-" * len(header))
    for zip_name in all_results:
        res = all_results[zip_name]
        statuses = []
        for fmt in FORMATS:
            name = fmt["name"]
            passed, desc = res.get(name, (False, "Not run"))
            status = "PASSED" if passed else "FAILED"
            width = 11 if name == "Freestyle15" else 10
            statuses.append(f"{status:<{width}}")
        print(f"{zip_name:<22} | {' | '.join(statuses)}")
    print("==========================================================================================")
