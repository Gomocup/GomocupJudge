...
/** parse coordinates x,y */
static void parse_2int_chk(const char *param, int *x, int *y)
{
...
static void do_command()
{
	...
	else if ((param = get_cmd_param("swap2board", cmd)) != 0) {
		int n = 0;
		start();
		for (;;) { /* fill the whole board */
			get_line();
			if (_stricmp(cmd, "done") == 0)
				break;
			parse_2int_chk(cmd, &x, &y);
			if (n % 2 == 0)
				brain_opponents(x, y);
			else
				brain_my(x, y);
